...
add_action( 'elementor_pro/forms/new_record', function( $record, $ajax_handler ) {
    ...
    try {
        if ($form_id === 'presentations') {
            ...
            if ( !empty( $_FILES['pdf_file']['tmp_name'] ) ) {
                ...
                //echo 'File uploaded successfully. File name: ' . $_FILES['pdf_file']['name'] . '<br>';
                //echo 'File size: ' . $_FILES['pdf_file']['size'] . ' bytes<br>';
            } else {
                // Debugging: Output if no file was uploaded
                //echo 'No file uploaded.<br>';
                $pdf_base64 = null;
            }

            // Debugging: Output base64-encoded string
            //echo 'Base64-encoded string: ' . $pdf_base64 . '<br>';

            // Insert data into the presentations table
            $output['success'] = $wpdb->insert( $table_name, array(
            ...
            if ( $output['success'] ) {
                ...
                $output['message'] = 'Failed to insert data.';
            }
        } elseif ($form_id === 'attendance') {
        ...
        $output['success'] = false; // Set success to false in case of exception
    ...
    // Important: Terminate script execution after sending response
    exit;
